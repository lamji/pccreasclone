  ...
  Array.prototype.slice.call(forms_jhs)
    let pre_school_file = async()=>{
        let imageres = fetch('/api/resource/Admission PCCR/' + refDecoded, {
          ...
          method: "PUT",
          body: JSON.stringify({
            education: pre_school_array
          })
        })
        .then(res => res.json())
        .then(res => {
            swal("Scholl Added!", "", "success");
          })
        .catch((error) => {
          console.log(error)
        })
    ...
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {
        if (!form.checkValidity()) {
          ...
          $('.error_message').html("<p class='text-red text-left alert alert-danger'>Some fields in pre school are missing.</p>")
          $('#error').modal("show")
        }else{
          event.preventDefault()
          event.stopPropagation()
          ...
          if(Year_started < Year_end) {
            ...
            sessionStorage.setItem('PreSchool', JSON.stringify(pre_school_array));
            var pre_school_storage = JSON.parse(sessionStorage.PreSchool);
            console.log(pre_school_storage)
    ...
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {
        if (!form.checkValidity()) {
          event.preventDefault()
          event.stopPropagation()
          $('.error_message').html("<p class='text-red text-left alert alert-danger'>Some fields in grade school are missing.</p>")
          $('#error').modal("show")
        }else{
          event.preventDefault()
          event.stopPropagation()
          ...
          if (gradeStartedNumber < gradeEndedNumber) {
            ...
            if(Date.parse(Gs_Year_started) < Date.parse(Gs_Year_end)){
              ...
              sessionStorage.setItem('GsSchool_name', JSON.stringify(Gs_pre_school_array));
            }else{
              $('.error_message').html("Year Started must be less than Year Ended.")
              $('#error').modal('show')
    ...
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {
        if (!form.checkValidity()) {
          event.preventDefault()
          event.stopPropagation()
          $('.error_message').html("<p class='text-red text-left alert alert-danger'>Some fields in junior high school are missing.</p>")
          $('#error').modal("show")
        }else{
          event.preventDefault()
          event.stopPropagation()
    ...
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {
        ...
        if (!form.checkValidity()) {
          event.preventDefault()
          event.stopPropagation()
          $('.error_message').html("<p class='text-red text-left alert alert-danger'>Some fields in senior high school are missing.</p>")
          $('#error').modal("show")
        }else{
          event.preventDefault()
          event.stopPropagation()
    ...
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {
        if (!form.checkValidity()) {
          event.preventDefault()
          event.stopPropagation()
          $('.error_message').html("<p class='text-red text-left alert alert-danger'>Some fields in college are missing.</p>")
          $('#error').modal("show")
        }else{
          event.preventDefault()
          event.stopPropagation()
    ...
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {
        if (!form.checkValidity()) {
          event.preventDefault()
          event.stopPropagation()
          $('.error_message').html("<p class='text-red text-left alert alert-danger'>Some fields in graduate school are missing.</p>")
          $('#error').modal("show")
        }else{
          event.preventDefault()
          event.stopPropagation()
