...
// frappe.ready(function() {
    ...
    const personal_info_Tab = `
		...
		.then(res => {
			...
			$('#city option').html(`<option class="city_temp" id="${res.data.city_code}" selected="" disabled="">${res.data.municipalitycity}</option>`)
			$('#barangay option').html(`<option class="bragy_temp" " selected="" disabled="">${res.data.barangay}</option>`)
			$('#province option').html(`<option class="province_temp" id="${res.data.province_code}selected="" disabled="">${res.data.province}</option>`)
			}
